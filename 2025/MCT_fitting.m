...
%         - packCurrent -> cellCurrent (/2) & 부호반전(방전 시 음수)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% 3) fmincon 옵션 및 파라미터 초기값/제약조건 설정 (예시)
%  5개 파라미터: X = [R0, R1, C1, R2, C2]
lb = [0,   0,    0,   0,    0   ];
ub = [0.2, 0.5, 2000, 0.5, 2000];
x0 = [0.01,0.02, 100, 0.02, 100];  % 초기 추정값 (임의 예시)
...
% (b) "셀 전류"로 변환 (2병렬)
%     그리고 방전 시 음수화 => OCV - I·R 형태와 부호 일치
cellCurrent = packCurrent / 2;   

% 초기 SOC 설정 (OCV 기반)
SOC0 = OCVMCT.OCV_SoC(mctNumber);
dt = [0; diff(time_s)];  % 시계열 간격
...
bestParams = paramArray(idxMin,:);  % 5개
bestR0 = bestParams(1);
bestR1 = bestParams(2);
bestC1 = bestParams(3);
bestR2 = bestParams(4);
...
title('Error = (Measured - Modeled)');
grid on;

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 서브펑션 (RMSE 계산 + 2-RC 모델 전압 계산)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function V_est = modelVoltage_2RC(X, SOC_t, I_cell, dt, socOCV, ocvCellVoltage)
    ...
    for k = 1:N
        ...
        % (2) R0 강하 (I_cell(k)는 이미 방전 시 음수화)
        IR0 = R0 * I_cell(k);
